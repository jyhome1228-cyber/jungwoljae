(()=>{
  ...
  const fillYears=id=>{const el=document.getElementById(id);for(let y=currentYear;y>=1930;y--)el.add(new Option(`${y}년`,String(y)));};
  ...
  function setupPerson(prefix){
    const ids={
      year:`${prefix}year`,month:`${prefix}month`,day:`${prefix}day`,
      meridiem:`${prefix}meridiem`,hour:`${prefix}hour`,minute:`${prefix}minute`,
      city:`${prefix}city`,calendar:`${prefix}calendar`,leap:`${prefix}leap`,lunarWrap:`${prefix}lunar-wrap`
    };
    ...
    const lunarWrap=document.getElementById(ids.lunarWrap);
  ...
  function getTime(prefix){
    ...
    let hour=Number(hh)%12;if(mer==='pm')hour+=12;
    return `${pad(hour)}:${mm}`;
  }
  ...
  form.addEventListener('submit',event=>{
    ...
    const a={
      ...
      birthTime:getTime('birth-'),
      calendarType:document.getElementById('birth-calendar').value,
      isLeapMonth:document.getElementById('birth-leap').checked,
      gender:document.getElementById('birth-gender').value,
      city:document.getElementById('birth-city').value
    };
    const b={
      name:form.elements.partnerName.value.trim(),
      birthDate:getDate('partner-birth-'),
      birthTime:getTime('partner-birth-'),
      calendarType:document.getElementById('partner-birth-calendar').value,
      isLeapMonth:document.getElementById('partner-birth-leap').checked,
      gender:document.getElementById('partner-birth-gender').value,
      city:document.getElementById('partner-birth-city').value
    };
