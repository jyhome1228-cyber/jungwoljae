...
function setSync(state,label,detail=''){ensureSyncUi();const pill=document.querySelector('[data-admin-sync-pill]');const detailNode=document.querySelector('[data-admin-sync-detail]');if(pill){pill.dataset.state=state;pill.textContent=label;}if(detailNode)detailNode.textContent=detail||firebaseConfig.projectId||'';}
ensureSyncUi();
function isUnlocked(){try{return sessionStorage.getItem(SESSION_KEY)==='1';}catch(e){return false;}}
function setUnlocked(value){try{if(value)sessionStorage.setItem(SESSION_KEY,'1');else sessionStorage.removeItem(SESSION_KEY);}catch(e){}}
async function openDashboard(){setUnlocked(true);document.body.classList.add('admin-unlocked');gate.hidden=true;dashboard.hidden=false;passStatus.textContent='';if(!dashboardLoaded){dashboardLoaded=true;await loadDashboard();}if(adminAuthorized&&!readingsLoaded)await loadReadingData();}
function closeDashboard(){setUnlocked(false);document.body.classList.remove('admin-unlocked');dashboard.hidden=true;gate.hidden=false;passInput.value='';passStatus.textContent='';closeReadingModal();setTimeout(()=>passInput.focus(),50);}
passForm?.addEventListener('submit',async event=>{event.preventDefault();const value=String(passInput.value||'').trim();if(value!==ADMIN_CODE){passStatus.textContent='운영 코드가 맞지 않습니다.';passInput.select();return;}passStatus.textContent='확인되었습니다.';await openDashboard();});
...
function showReadingAuth(kind){
  ...
  const map={
    ...
    login:['운영 계정 로그인이 필요합니다.','이 영역에는 이름·생년월일·출생시간 등 입력 정보가 포함되어 있어 관리자 계정으로 로그인한 경우에만 표시합니다.','<a href="./login.html?next=./admin.html">운영 계정 로그인</a>'],
  ...
  const [title,desc,action]=map[kind]||map.error;
  readingAuthState.innerHTML=`<strong>${title}</strong><p>${desc}</p>${action}`;
}

async function resolveAdmin(user){
  adminUser=user||null;adminAuthorized=false;readingsLoaded=false;
  document.querySelector('[data-metric-readings-today]').textContent='—';
  document.querySelector('[data-metric-readings-total]').textContent='—';
  if(!user){showReadingAuth('login');return;}
  showReadingAuth('checking');
  try{
    const snap=await getDoc(doc(db,'admins',user.uid));
    if(!snap.exists()){showReadingAuth('denied');return;}
    adminAuthorized=true;
    readingAuthState.hidden=true;
    readingPrivateContent.hidden=false;
    if(isUnlocked())await loadReadingData();
  }catch(error){console.error('[Jungwoljae Admin auth]',error);showReadingAuth('error');}
}
...
async function loadReadingData(){
  if(!adminAuthorized||!isUnlocked())return;
  readingPrivateContent.hidden=false;readingAuthState.hidden=true;
  ...
  try{
    ...
    document.querySelector('[data-metric-readings-today]').textContent=n(todaySnap.data().count||0);
    document.querySelector('[data-metric-readings-total]').textContent=n(totalSnap.data().count||0);
    readingsLoaded=true;
    renderReadingTable();
  }catch(error){
...
async function openReadingDetail(id){
  const d=readingEvents.find(x=>x.id===id);if(!d||!readingModal||!readingDetail)return;
  ...
  let member=null;
  if(d.uid&&adminAuthorized){try{const snap=await getDoc(doc(db,'users',d.uid));if(snap.exists())member=snap.data();}catch(e){}}
...
async function loadDashboard(){if(!isUnlocked())return;refreshBtn.disabled=true;refreshBtn.textContent='집계 중…';setSync('','Firebase 연결 중',firebaseConfig.projectId||'');try{const dates=[-6,-5,-4,-3,-2,-1,0].map(seoulDateKey);const reviewPromise=getDocs(collection(db,'reviews'));const trafficPromise=Promise.all(dates.map(loadTrafficDay));const [reviewSnap,days]=await Promise.all([reviewPromise,trafficPromise]);const today=days.at(-1);const reviews=[...reviewSnap.docs].sort((a,b)=>(toDate(b.data().createdAt)?.getTime()||0)-(toDate(a.data().createdAt)?.getTime()||0));const avg=reviews.length?reviews.reduce((sum,s)=>sum+Number(s.data().rating||0),0)/reviews.length:0;const weekPageViews=days.reduce((sum,d)=>sum+d.pageViews,0),weekVisitors=days.reduce((sum,d)=>sum+d.visitors,0);document.querySelector('[data-metric-pageviews]').textContent=n(today.pageViews);document.querySelector('[data-metric-visitors]').textContent=n(today.visitors);document.querySelector('[data-metric-week-pageviews]').textContent=n(weekPageViews);document.querySelector('[data-metric-week-visitors]').textContent=n(weekVisitors);document.querySelector('[data-metric-reviews]').textContent=n(reviews.length);document.querySelector('[data-metric-rating]').textContent=reviews.length?avg.toFixed(1):'—';document.querySelector('[data-admin-updated]').textContent=new Intl.DateTimeFormat('ko-KR',{hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date());renderTraffic(days);renderServiceUsage(days);renderPages(today.pages,today.pageViews);renderReviews(reviews);const anyTraffic=weekPageViews>0||weekVisitors>0;setSync('ok',anyTraffic?'Firebase 연동 정상':'Firebase 연결 정상 · 집계 대기',`${firebaseConfig.projectId} · ${seoulDateKey()}`);if(adminAuthorized)await loadReadingData();}catch(error){console.error('[Jungwoljae Admin]',error);document.querySelector('[data-admin-updated]').textContent='집계 실패';setSync('warn','데이터 읽기 실패',firestoreHint(error));const traffic=document.querySelector('[data-traffic-chart]');if(traffic)traffic.innerHTML=`<div class="admin-data-empty" style="grid-column:1/-1"><strong>Firebase 데이터를 읽지 못했습니다.</strong>${esc(firestoreHint(error))}</div>`;}finally{refreshBtn.disabled=false;refreshBtn.textContent='새로고침';}}
refreshBtn?.addEventListener('click',async()=>{if(!isUnlocked())return;readingsLoaded=false;await loadDashboard();});
if(isUnlocked())openDashboard();else{document.body.classList.remove('admin-unlocked');gate.hidden=false;dashboard.hidden=true;setTimeout(()=>passInput?.focus(),50);}
