...
async function openDashboard(){setUnlocked(true);document.body.classList.add('admin-unlocked');gate.hidden=true;dashboard.hidden=false;passStatus.textContent='';if(!dashboardLoaded){dashboardLoaded=true;await loadDashboard();}}
function closeDashboard(){setUnlocked(false);document.body.classList.remove('admin-unlocked');dashboard.hidden=true;gate.hidden=false;passInput.value='';passStatus.textContent='';setTimeout(()=>passInput.focus(),50);}
