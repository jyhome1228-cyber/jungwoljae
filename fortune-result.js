...
const forDay=value=>String(value??'').replaceAll('오늘',dayWord);
...
function decisiveAreaCopy(v){
  ...
  const tone=favorable?'favorable':tense?'tense':'steady';
  const loveTone=connected?'favorable':conflicted?'tense':'steady';
  const variantFor=area=>hash(`${input.birthDate}|${input.birthTime||''}|${v.expected}|${area}`)%2;
  const base={
    money:favorable
      ...
      :tense
        ?`돈이 들어오는 속도보다 나가는 속도가 빨라질 수 있는 날입니다. 예상하지 못한 비용이나 추가 결제가 생길 수 있으니 큰 지출과 신규 투자는 미루는 편이 안전합니다. ${dayWord}은 수익을 늘리는 것보다 손실을 막고 미수금과 고정비를 정리하는 것이 중요합니다.`
    ...
    love:connected
      ...
      :conflicted
        ?`상대의 말과 행동을 다르게 받아들여 오해가 생기기 쉬운 날입니다. 답장이 늦거나 표현이 무뚝뚝하더라도 성급하게 마음을 단정하지 마세요. ${dayWord}은 감정을 쏟아내기보다 사실을 직접 확인하는 대화가 관계를 지켜줍니다.`
    ...
    health:tense
      ?`큰 질환보다 피로와 스트레스가 몸으로 나타나기 쉬운 날입니다. 소화불량, 두통, 목과 어깨의 긴장처럼 신경성 증상을 가볍게 넘기지 마세요. 식사를 거르거나 늦게까지 무리하면 회복이 더뎌질 수 있으니 ${dayWord}은 휴식 시간을 먼저 확보해야 합니다.`
  ...
  const alternate={
    money:{
      favorable:`재물과 관련된 선택에서 실속을 얻기 좋은 날입니다. 새로운 수입의 가능성을 찾거나 협상과 견적 조정을 진행한다면 내가 원하는 조건을 분명히 제시해보세요. 특히 그동안 미뤄졌던 입금, 환급, 정산 문제를 다시 확인하면 예상보다 빠른 답을 받을 수 있습니다. 다만 운이 좋다는 이유로 계획에 없던 투자나 소비까지 늘리기보다 실제로 확보된 금액 안에서 움직여야 좋은 흐름을 지킬 수 있습니다.`,
      steady:`재물운은 크게 흔들리지 않지만 작은 지출이 여러 번 겹치면 생각보다 많은 돈이 빠져나갈 수 있습니다. 수입을 새로 만들기보다 구독료, 반복 결제, 미처 확인하지 못한 비용을 점검하는 데 유리한 날입니다. 거래나 구매를 결정해야 한다면 가격만 보지 말고 유지비와 실제 사용 기간까지 함께 비교하세요. 당장의 만족보다 오래 남는 선택을 할 때 금전적인 안정감이 커집니다.`,
      tense:`재물 문제에서는 욕심과 불안이 동시에 커질 수 있는 날입니다. 손해를 만회하려고 더 큰 금액을 움직이거나 다른 사람의 수익 이야기를 듣고 서둘러 따라가면 판단이 흐려질 수 있습니다. 계약금, 대출, 투자, 고가 구매처럼 되돌리기 어려운 결정은 하루 정도 시간을 두는 편이 좋습니다. 꼭 지출해야 한다면 한도를 먼저 정하고 추가 비용이 생길 가능성까지 계산해야 합니다.`
    },
    love:{
      favorable:`사람 사이의 거리가 자연스럽게 가까워지는 날입니다. 새로운 인연은 공통된 관심사나 일상적인 대화에서 편안하게 이어질 가능성이 높고, 연인이 있다면 미뤄둔 약속을 구체적으로 잡기에 좋습니다. 상대의 마음을 확인하려고 시험하기보다 내가 느끼는 호감과 고마움을 솔직하게 표현해보세요. 다만 관계를 빠르게 규정하려 하지 말고 상대가 반응할 시간을 남겨주는 것이 좋습니다.`,
      steady:`애정운은 잔잔하게 이어지지만 상대가 먼저 알아주기를 기다리면 서운함이 쌓일 수 있습니다. 연인이 있다면 익숙하다는 이유로 대화를 줄이지 말고 최근 서로 바빴던 부분을 편안하게 이야기해보세요. 솔로라면 강한 설렘보다 함께 있을 때 부담이 적은 사람을 유심히 보는 것이 좋습니다. 관계의 가능성은 화려한 말보다 연락과 행동이 꾸준히 이어지는지에서 확인할 수 있습니다.`,
      tense:`관계에서는 작은 말도 평소보다 날카롭게 들릴 수 있는 날입니다. 과거의 문제를 다시 꺼내거나 상대의 답을 재촉하면 본래 의도보다 갈등이 커질 수 있습니다. 감정이 올라오는 순간 바로 결론을 내리기보다 무엇이 사실이고 무엇이 내 추측인지 먼저 나눠보세요. 꼭 해야 할 이야기가 있다면 비난하는 표현보다 내가 불편했던 상황과 원하는 바를 구체적으로 전달하는 것이 좋습니다.`
    },
    work:{
      favorable:`대외적인 일에서 존재감과 신뢰를 얻기 좋은 날입니다. 새 거래처와의 연락, 중요한 미팅, 제안서 전달처럼 상대의 판단을 끌어내야 하는 일을 앞쪽에 배치해보세요. 준비한 근거와 결과물을 분명하게 보여주면 말만 길게 설명할 때보다 좋은 반응을 얻을 수 있습니다. 다만 기회가 여러 개 들어오더라도 일정과 인력을 확인하지 않고 모두 수락하면 품질이 흔들릴 수 있으니 우선순위를 정해야 합니다.`,
      steady:`직장과 사업에서는 눈에 띄는 변화보다 신뢰를 쌓는 과정이 중요한 날입니다. 새로운 제안을 급하게 꺼내기보다 진행 중인 업무의 일정, 역할, 결과 기준을 다시 정리하면 이후의 마찰을 줄일 수 있습니다. 상대방과 의견이 다르다면 누가 맞는지를 따지기보다 최종적으로 필요한 결과가 무엇인지부터 맞추세요. 작은 업무라도 약속한 시간 안에 정확하게 마무리하는 것이 다음 기회로 이어집니다.`,
      tense:`직장이나 사업에서 경쟁 관계와 이해관계가 예민하게 드러날 수 있는 날입니다. 거래처나 동료의 말을 개인적인 공격으로 받아들이면 불필요한 시비가 길어질 수 있으니 감정보다 계약 조건과 업무 범위를 기준으로 대응하세요. 중요한 결정은 구두 합의만 믿지 말고 자료와 기록을 남겨야 합니다. 무리하게 성과를 증명하려 하기보다 문제가 될 수 있는 부분을 먼저 찾아 막는 것이 유리합니다.`
    },
    study:{
      favorable:`학업과 준비 과정에서 노력한 만큼 결과를 확인하기 좋은 날입니다. 시험, 발표, 면접, 포트폴리오처럼 그동안 쌓아온 내용을 보여줘야 한다면 핵심을 먼저 제시하고 세부 설명을 붙이는 방식이 효과적입니다. 새로운 지식을 받아들이는 속도도 좋은 편이므로 어렵게 느껴졌던 주제를 다시 살펴보세요. 혼자 공부한 내용을 다른 사람과 점검하면 놓쳤던 부분까지 채울 수 있습니다.`,
      steady:`학업운은 무난하지만 목표가 모호하면 공부한 시간에 비해 남는 것이 적을 수 있습니다. 시작하기 전에 이번 시간에 읽을 범위, 풀 문제, 완성할 문장을 구체적으로 정해두세요. 진로와 취업 문제는 막연한 걱정보다 필요한 자격, 일정, 비용을 표로 정리하면 판단하기 쉬워집니다. 많은 자료를 모으기보다 지금 가진 자료 한 개를 끝까지 이해하는 편이 성취감을 높여줍니다.`,
      tense:`학업과 성취에서는 조급함 때문에 계획을 자주 바꾸기 쉬운 날입니다. 다른 사람의 진도나 결과와 비교하면 내가 준비해 온 방향까지 의심하게 될 수 있으니 오늘 해야 할 최소 목표만 지키세요. 집중이 끊겼을 때 새 계획을 세우기보다 틀린 문제와 미완성 과제를 다시 확인하는 편이 좋습니다. 제출이나 시험 일정은 착각할 가능성이 있으므로 날짜와 파일을 반드시 재확인해야 합니다.`
    },
    health:{
      favorable:`몸의 회복력이 비교적 좋은 날입니다. 미뤄두었던 운동이나 생활 리듬을 다시 시작하면 몸이 가볍게 반응할 수 있습니다. 강도를 갑자기 높이기보다 가벼운 유산소 운동과 스트레칭으로 순환을 돕고, 식사 시간을 일정하게 맞춰보세요. 컨디션이 좋다고 밤늦게까지 무리하지만 않으면 하루의 기력을 안정적으로 유지할 수 있습니다.`,
      steady:`건강운은 평이하지만 같은 자세를 오래 유지하면 허리와 목, 어깨에 피로가 쌓일 수 있습니다. 한 번에 오래 운동하기보다 일하는 사이마다 잠깐씩 일어나 몸을 움직이는 것이 더 도움이 됩니다. 소화가 더디게 느껴질 수 있으니 늦은 시간의 과식과 자극적인 음식은 줄이세요. 수분을 충분히 섭취하고 수면 시간을 지키면 컨디션이 빠르게 회복됩니다.`,
      tense:`몸 자체의 문제보다 신경을 많이 쓰면서 생기는 피로를 조심해야 하는 날입니다. 가슴이 답답하거나 속이 불편하고 잠이 쉽게 오지 않는다면 계속 일을 붙잡기보다 자극을 줄이고 쉬어야 합니다. 과음, 야식, 과도한 카페인은 위장과 수면의 부담을 키울 수 있습니다. 증상이 계속되거나 심해진다면 운세 해석에 의존하지 말고 의료진의 진료를 받는 것이 필요합니다.`
    }
  };
  ['money','work','study'].forEach(area=>{if(variantFor(area))base[area]=alternate[area][tone];});
  if(variantFor('love'))base.love=alternate.love[loveTone];
  const healthTone=favorable?'favorable':tense?'tense':'steady';
  if(favorable){
    base.health=variantFor('health')
...
function tomorrowPreparation(v){
  if(!tomorrow)return '';
  const variant=hash(`${input.birthDate}|${input.birthTime||''}|${v.expected}|tomorrow-prep`)%2;
  if(v.score>=58)return variant
    ?`내일은 사람과 기회가 연결될 가능성이 높습니다. 오늘 밤에는 답변해야 할 연락과 보여줄 자료를 한곳에 모아두고, 내일 먼저 제안할 내용을 한 문장으로 정리해보세요. 일정 사이에 여백을 남겨두면 갑자기 들어오는 좋은 제안에도 여유 있게 대응할 수 있습니다.`
    :`내일은 준비한 것을 밖으로 보여줄수록 흐름이 살아납니다. 오늘 밤에는 내일 가장 먼저 꺼낼 자료와 연락할 사람, 반드시 끝낼 일 한 가지를 미리 정해두세요. 기회가 들어왔을 때 바로 움직일 수 있도록 일정에 작은 여백을 남겨두는 것도 좋습니다.`;
  if(v.score<44)return variant
    ?`내일은 서두르는 순간 작은 실수가 커질 수 있습니다. 오늘 밤에는 이동 시간, 결제 한도, 제출 파일처럼 틀리기 쉬운 항목을 체크리스트로 확인하세요. 중요한 대화는 즉시 결론을 내리지 않아도 되도록 필요한 근거를 미리 준비해두는 것이 안전합니다.`
    :`내일은 일정 변경이나 감정적인 반응처럼 예상 밖의 변수가 생길 수 있습니다. 오늘 밤에는 중요한 약속과 제출 시간을 다시 확인하고, 필요한 자료와 이동 준비를 미리 끝내두세요. 결정을 서두르지 않아도 되도록 선택지를 줄여두면 내일의 실수를 크게 줄일 수 있습니다.`;
  return variant
    ?`내일은 해야 할 일을 늘리기보다 이미 정한 계획을 정확하게 실행하는 편이 좋습니다. 오늘 밤에는 일정과 준비물을 가볍게 점검하고, 오전에 집중할 시간을 미리 비워두세요. 첫 행동이 분명하면 주변의 요청이 생겨도 내 흐름을 잃지 않을 수 있습니다.`
    :`내일은 크게 무리하기보다 정해둔 일을 정확하게 처리할 때 안정적인 흐름을 얻을 수 있습니다. 오늘 밤에는 내일의 우선순위를 세 가지 이내로 줄이고, 오전에 시작할 일 하나를 미리 펼쳐두세요. 준비가 단순할수록 내일 집중력이 오래 유지됩니다.`;
}
...
async function build(){
  ...
  let stemText=`${dayWord} 날짜의 흐름이 내 선택을 크게 방해하지 않는 편입니다.`;
  if(stemA&&stemB){
    if(generates[stemB.e]===stemA.e)stemText=`${dayWord} 날짜의 흐름이 내 준비와 회복을 받쳐주는 편입니다.`;
...
function renderFull(v){
  ...
  $('[data-summary]').textContent=tomorrow
    ...
    :`${name}님의 연·월·일${v.h?'·시':''} 흐름과 오늘 날짜를 함께 보았습니다. ${v.godPack.title}이 두드러지고, ${v.impact.copy}`;
  ...
  if(relation)relation.innerHTML=`
    ...
    <p>${esc(`${forDay(v.godPack.action)} 관계에서는 ${v.dayRel.copy} 일에서는 ${v.monthRel.copy}`)}</p>`;
  ...
  $('[data-fortune-story]').innerHTML=`
    ...
    <p><strong>개인적인 흐름에서는</strong> ${esc(forDay(v.godPack.action))} ${esc(v.dayRel.copy)} ${v.h?esc(`하루 후반에는 ${v.hourRel.copy}`):`저녁에는 새로운 일을 더 벌이기보다 ${dayWord} 처리한 내용을 정리하고 몸과 마음의 긴장을 풀어주는 것이 좋습니다.`}</p>
    ${tomorrow?`<p><strong>오늘 미리 준비할 것</strong> ${esc(tomorrowPreparation(v))}</p>`:''}`;
  ...
  const morning=`${forDay(v.godPack.action)} 오전에는 연락과 알림부터 확인하기보다 ${dayWord} 반드시 끝낼 일 한 가지를 먼저 정하세요. 판단력이 비교적 선명한 시간에 문서 작성, 공부, 중요한 결정을 배치하면 흐름을 효율적으로 사용할 수 있습니다.`;
  ...
  const evening=v.h
    ?`${relationSentence(v.hourRel.label,'work')} 저녁에는 낮 동안 미뤄둔 감정이나 피로가 올라올 수 있습니다. 새 일을 벌이기보다 ${dayWord} 처리한 내용을 정리하고, 짧게 몸을 움직인 뒤 평소보다 일찍 쉬는 편이 좋습니다.`
    :`저녁에는 ‘${balancePack[v.supportEl].keyword}’을 보완하는 휴식이나 정리 시간을 남겨보세요. 낮에 해결하지 못한 문제를 억지로 붙잡기보다 다음 날 가장 먼저 할 일을 적어두고 생각을 멈추는 것이 회복에 도움이 됩니다.`;
  ...
  const positives=[
    [forDay(v.godPack.action),`${v.godPack.title}을 가장 실용적으로 쓰는 행동입니다.`],
  ...
  $('[data-key-grid]').innerHTML=`
    <article class="fortune-key-card is-action"><span>01 · 가장 먼저</span><strong>${esc(forDay(v.godPack.action))}</strong><p>${esc(v.godPack.title)}이 중심에 들어오는 날입니다.</p><small>개인 성향 × ${dayWord}의 흐름</small></article>
  ...
  if(lucky)lucky.innerHTML=`
    ...
    <div class="fortune-lucky-grid">
      <article class="fortune-lucky-card"><small>보완 키워드</small><strong>${esc(bp.keyword)}</strong><p>${dayWord} 일부러 조금 더 써보면 좋은 행동 방향입니다.</p></article>
      ...
      <article class="fortune-lucky-card"><small>${dayWord}의 균형</small><strong>강점은 살리고, 부족한 쪽은 조금만 보완</strong><p>잘하는 것을 억지로 줄이기보다 부족한 행동 하나를 더하는 방식이 좋습니다.</p></article>
    </div>`;
...
function renderFallback(v){
  ...
  $('[data-fortune-story]').innerHTML=`<p><strong>${dayWord}의 전체 흐름</strong> ${esc(v.pack.headline)}. ${esc(v.dayRel.copy)}</p><p><strong>실제로는</strong> ${esc(v.pack.work)} ${esc(v.stemText)}</p>${tomorrow?`<p><strong>오늘 미리 준비할 것</strong> ${esc(tomorrowPreparation(v))}</p>`:''}`;
  ...
  $('[data-dont-list]').innerHTML=listItem('한꺼번에 너무 많은 일을 벌이기',`${dayWord}의 중심이 흐려질 수 있습니다.`,1)+listItem('한 번의 반응만 보고 결론 내리기','관계와 일 모두 전체 흐름을 같이 보세요.',2)+listItem('기분에 따라 지출 범위 넓히기','결제 전 목적과 예산을 한 번 확인하세요.',3);
  $('[data-key-grid]').innerHTML=`
    <article class="fortune-key-card is-action"><span>01 · 가장 먼저</span><strong>${esc(v.pack.work)}</strong><p>${dayWord}의 첫 행동을 분명하게 만들어보세요.</p></article>
    <article class="fortune-key-card is-action"><span>02 · 사람과 대화</span><strong>${esc(v.pack.love)}</strong><p>${esc(v.dayRel.copy)}</p></article>
    <article class="fortune-key-card is-action"><span>03 · 마무리</span><strong>${esc(v.pack.life)}</strong><p>${dayWord} 시작한 것 하나에 완료 표시를 남겨보세요.</p></article>`;
  const lucky=$('.fortune-lucky-section');
