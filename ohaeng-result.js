...
let currentUser=null;let reportData=null;let saved=false;
...
function parseTime(){if(input.birthTimeUnknown||!input.birthTime)return {hour:12,minute:0,known:false};const [h,m]=input.birthTime.split(':').map(Number);return {hour:h,minute:m,known:true};}
...
function elementFromPillar(pillar,score,weightStem=1,weightBranch=1.1){if(!pillar)return;const chars=[...pillar];addElement(score,stemMap[chars[0]],weightStem);addElement(score,branchMap[chars[1]],weightBranch);}
function round1(n){return Math.round(n*10)/10;}
function normalize(score){const total=Object.values(score).reduce((a,b)=>a+b,0)||1;const out={};Object.keys(elementInfo).forEach(k=>out[k]=round1((score[k]||0)/total*100));const diff=100-Object.values(out).reduce((a,b)=>a+b,0);out[Object.keys(out)[0]]=round1(out[Object.keys(out)[0]]+diff);return out;}
function sortedKeys(pct){return Object.keys(pct).sort((a,b)=>pct[b]-pct[a]);}
function balanceIndex(pct){const vals=Object.values(pct);const mean=20;const variance=vals.reduce((a,v)=>a+(v-mean)**2,0)/vals.length;return Math.max(0,Math.round(100-Math.sqrt(variance)*3));}

function calc(){
  const [y,m,d]=input.birthDate.split('-').map(Number);const t=parseTime();
  const result=calculateFourPillars({year:y,month:m,day:d,hour:t.hour,minute:t.minute,isLunar:input.calendarType==='lunar',isLeapMonth:Boolean(input.isLeapMonth),gender:input.gender||undefined});
  const pillars=pillarObject(result);
  const score={wood:0,fire:0,earth:0,metal:0,water:0};
  elementFromPillar(pillars.year,score);elementFromPillar(pillars.month,score);elementFromPillar(pillars.day,score);if(t.known)elementFromPillar(pillars.hour,score);
  if(pillars.month){const branch=[...pillars.month][1];addElement(score,branchMap[branch],0.9);}
  const pct=normalize(score);const order=sortedKeys(pct);const strongest=order.slice(0,2);const weakest=order[order.length-1];
  return {result,pillars,score,pct,order,strongest,weakest,balance:balanceIndex(pct),timeKnown:t.known};
}

function buildSummary(data){const [a,b]=data.strongest;const w=data.weakest;return `${input.name}님의 명식에서는 ${elementInfo[a].name}(${elementInfo[a].hanja})와 ${elementInfo[b].name}(${elementInfo[b].hanja})의 흐름이 상대적으로 두드러집니다. 반면 ${elementInfo[w].name}(${elementInfo[w].hanja})의 작용은 다른 기운에 비해 의식적으로 보완해 볼 부분으로 읽힙니다. 이 값은 좋고 나쁨의 점수가 아니라, 다섯 기운이 서로 어느 정도의 비중과 연결을 갖는지 보기 위한 상대 지표입니다.`;}
function buildStrong(data){return `<p>${elementInfo[data.strongest[0]].strong}</p><p>${elementInfo[data.strongest[1]].strong}</p>`;}
function buildWeak(data){const w=elementInfo[data.weakest];return `<p>${w.weak}</p><p>부족한 기운을 단순히 채워야 한다는 뜻은 아닙니다. ${w.name}의 역할이 필요한 상황에서 어떤 방식으로 균형을 만들지 살펴보는 것이 더 중요합니다.</p>`;}
function buildFlow(data){const a=elementInfo[data.strongest[0]],w=elementInfo[data.weakest];return `${a.name}(${a.hanja})의 작용이 비교적 자연스럽게 시작되는 반면 ${w.name}(${w.hanja})의 역할로 넘어가는 구간에서는 힘의 차이가 나타납니다. 따라서 이미 잘 쓰이는 힘을 더 키우기보다, 그 힘이 다음 단계로 이어질 수 있도록 정리·관찰·유지의 구조를 만들어주는 편이 전체 흐름을 안정시키는 데 도움이 됩니다.`;}
function lifeCards(data){const s=elementInfo[data.strongest[0]],w=elementInfo[data.weakest];return [
...
function questionAnswer(data){const q=input.question||'';const s=elementInfo[data.strongest[0]],w=elementInfo[data.weakest];let domain='현재 고민';if(/일|회사|직장|사업|퇴사|이직|업무/.test(q))domain='일과 선택';else if(/연애|관계|사람|인연|남자|여자|친구/.test(q))domain='관계';else if(/돈|재물|투자|수입|지출/.test(q))domain='재물과 생활';return `${domain}의 관점에서 보면, ${input.name}님은 ${s.name}의 방식인 ${s.keywords}을 먼저 사용하는 편으로 읽힙니다. 그래서 지금 고민에서도 움직임이나 판단이 빠르게 일어날 수 있습니다. 다만 이번 질문의 답을 길흉으로 단정하기보다, 상대적으로 약한 ${w.name}의 역할인 ${w.keywords}을 의도적으로 보완한 뒤 결정하는 편이 현재 오행 구조에는 더 안정적입니다. 질문에 대한 결론 자체보다 ‘어떤 방식으로 선택할 때 흔들림이 줄어드는가’를 참고해주세요.`;}
...
function reportText(data){
  const lines=[];lines.push(`정월재 오행 분석 리포트`,`대상: ${input.name}`,`기준: ${input.birthDate} · ${input.calendarType==='lunar'?'음력':'양력'} · ${data.timeKnown?input.birthTime:'출생시간 모름'}`,'');
  lines.push('1. 오행 요약',buildSummary(data),'');
  lines.push('2. 오행 상대 강도');data.order.forEach(k=>lines.push(`${elementInfo[k].name}(${elementInfo[k].hanja}) ${data.pct[k]}%`));lines.push('');
  ...
  lines.push('5. 오행의 흐름',buildFlow(data),'');
  lines.push('6. 생활 해석');lifeCards(data).forEach(x=>lines.push(`${x[1]}: ${x[2]}`));lines.push('');
  if(input.question){lines.push('7. 남겨주신 질문',input.question,questionAnswer(data),'');}
  lines.push('핵심 정리',`잘 쓰이는 힘: ${elementInfo[data.strongest[0]].name} · ${elementInfo[data.strongest[1]].name}`,`균형이 필요한 힘: ${elementInfo[data.weakest].name}`,`오행 균형 지표: ${data.balance}/100`,'','※ 본 리포트는 전통 명리학 이론을 바탕으로 한 해석 콘텐츠이며 미래 결과를 보장하지 않습니다.');return lines.join('\n');
}

function render(data){
  $('[data-name]').textContent=input.name;$('[data-summary]').textContent=buildSummary(data);
  ...
  $('[data-strong-text]').innerHTML=buildStrong(data);$('[data-weak-text]').innerHTML=buildWeak(data);$('[data-flow-text]').textContent=buildFlow(data);
  $('[data-life-grid]').innerHTML=lifeCards(data).map(([n,t,p])=>`<article class="life-card"><span>${n}</span><h3>${t}</h3><p>${p}</p></article>`).join('');
  if(input.question){const sec=$('[data-question-section]');sec.hidden=false;$('[data-question]').textContent=input.question;$('[data-question-answer]').textContent=questionAnswer(data);}
  const keyItems=[['01','잘 쓰이고 있는 힘',`${elementInfo[data.strongest[0]].name} · ${elementInfo[data.strongest[1]].name}`,`이미 자연스럽게 쓰이는 힘은 장점이지만 과해질 때의 부담도 함께 살펴봅니다.`],['02','균형이 필요한 힘',elementInfo[data.weakest].name,`${elementInfo[data.weakest].keywords}의 역할을 생활 속 구조로 보완해보세요.`],['03','전체 균형',`${data.balance}/100`,'점수가 높고 낮음의 길흉이 아니라 다섯 기운의 편차를 보기 위한 참고 지표입니다.']];
...
try{reportData=calc();render(reportData);}catch(error){root.innerHTML=`<div class="result-container"><section class="report-section"><p class="section-label">CALCULATION ERROR</p><h1 style="font-size:26px;margin:0 0 12px">오행 계산을 완료하지 못했습니다.</h1><p style="color:#756b67;line-height:1.7">입력한 날짜가 지원 범위를 벗어났거나 음력 정보가 올바르지 않을 수 있습니다. 입력 정보를 다시 확인해주세요.</p><a href="./ohaeng.html" class="result-button primary" style="display:inline-flex;align-items:center;text-decoration:none;margin-top:16px">입력 다시 하기</a></section></div>`;throw error;}

const copyBtn=$('[data-copy]'),pdfBtn=$('[data-pdf]'),saveBtn=$('[data-save]'),actionStatus=$('[data-action-status]');
copyBtn.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(reportText(reportData));actionStatus.textContent='보고서용 텍스트를 복사했습니다.';}catch(e){actionStatus.textContent='복사하지 못했습니다. 브라우저 권한을 확인해주세요.';}});
pdfBtn.addEventListener('click',()=>window.print());

onAuthStateChanged(auth,user=>{currentUser=user;if(!user){saveBtn.textContent='로그인 후 저장';}else saveBtn.textContent=saved?'저장 완료':'마이페이지에 저장';});
saveBtn.addEventListener('click',async()=>{
  if(saved){actionStatus.textContent='이미 마이페이지에 저장했습니다.';return;}
  if(!currentUser){location.href='./login.html?next=./ohaeng-result.html';return;}
  saveBtn.disabled=true;saveBtn.textContent='저장 중…';
  try{
    const data={type:'ohaeng',title:`${input.name}님의 오행 분석`,input,summary:buildSummary(reportData),pillars:reportData.pillars,elements:reportData.pct,strongest:reportData.strongest.map(k=>elementInfo[k].name),weakest:elementInfo[reportData.weakest].name,balanceIndex:reportData.balance,question:input.question||'',reportText:reportText(reportData),createdAt:serverTimestamp()};
    await addDoc(collection(db,'users',currentUser.uid,'readings'),data);saved=true;saveBtn.textContent='저장 완료';actionStatus.textContent='정월록에 오행 분석 결과를 저장했습니다.';
