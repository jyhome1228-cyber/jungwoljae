...
function openRequestedReading(){
  ...
  if(target){target.open=true;setTimeout(()=>target.scrollIntoView({behavior:'smooth',block:'center'}),120);}
}

onAuthStateChanged(auth,async user=>{
  if(!user){status.textContent='로그인하면 저장한 분석을 확인할 수 있습니다.';list.innerHTML='<div class="empty-record">정월록은 로그인 회원의 개인 기록 공간입니다. 사주 풀이와 정월도감의 고민 해답을 이곳에 차곡차곡 모아둘 수 있습니다.</div>';return;}
  try{
    ...
    list.innerHTML=snap.docs.map(docSnap=>{
      ...
      return `<details class="reading-card" id="reading-${esc(docSnap.id)}" data-reading-id="${esc(docSnap.id)}"><summary><span class="reading-type">${esc(typeLabel(d.type))}</span><div class="reading-title-wrap"><h3>${esc(d.title||'정월재 분석')}</h3>${question?`<p>${esc(question)}</p>`:''}</div><span class="reading-date">${esc(formatDate(d.createdAt))}</span></summary><div class="reading-body">${direct?`<div class="reading-direct"><span>질문에 대한 답</span><strong>${esc(direct)}</strong></div>`:''}<pre class="reading-report">${esc(report)}</pre></div></details>`;
    }).join('');
