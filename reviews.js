...
import { getFirestore, collection, query, orderBy, limit, getDocs, onSnapshot, addDoc, serverTimestamp, doc, getDoc } from 'https://www.gstatic.com/firebasejs/12.18.0/firebase-firestore.js';
...
const PAGE_SIZE=6;
let currentUser=null;let currentProfile=null;let reviewableReadings=[];
...
const TYPE_LABELS={ohaeng:'오행 분석',saju:'종합 사주',fortune:'오늘의 운세',relationship:'연애와 인연',compatibility:'궁합',work:'일과 재물','work-money':'일과 재물',guide:'정월도감',tomorrow:'내일의 운세','lucky-number':'행운의 숫자','important-day':'중요한 날',moving:'이사 택일',talisman:'정월부적'};
...
function formatDate(value){try{const date=value?.toDate?value.toDate():value?new Date(value):null;if(!date)return '방금';return new Intl.DateTimeFormat('ko-KR',{year:'numeric',month:'2-digit',day:'2-digit'}).format(date).replace(/\s/g,'');}catch(e){return '방금';}}
function anonymousLabel(profile){const year=String(profile?.birthDate||'').slice(2,4);const gender=profile?.gender==='male'?'남성':profile?.gender==='female'?'여성':'회원';return year?`${year}년생 ${gender}`:`정월재 ${gender}`;}
function cardMarkup(review){return `<article class="review-entry"><div class="review-entry-top"><span class="review-stars" aria-label="${Number(review.rating)||0}점">${stars(review.rating)}</span><span class="review-badge">${esc(review.service||'정월재')}</span></div><p>${esc(review.content||'')}</p><div class="review-entry-meta"><span class="review-entry-user">${esc(review.authorLabel||'정월재 회원')} · ${esc(review.service||'정월재')}</span><span>${esc(formatDate(review.createdAt))}</span></div></article>`;}
function withSamples(realItems=[]){const real=[...realItems];const needed=Math.max(0,20-real.length);return [...real,...SAMPLE_REVIEWS.slice(0,needed)].sort((a,b)=>new Date(b.createdAt?.toDate?b.createdAt.toDate():b.createdAt||0)-new Date(a.createdAt?.toDate?a.createdAt.toDate():a.createdAt||0));}

async function loadProfile(user){if(!user){currentProfile=null;return;}try{const snap=await getDoc(doc(db,'users',user.uid));currentProfile=snap.exists()?snap.data():null;}catch(e){currentProfile=null;}}
async function loadReviewableReadings(user){
  reviewableReadings=[];
  if(!user)return reviewableReadings;
  try{
    const q=query(collection(db,'users',user.uid,'readings'),orderBy('createdAt','desc'),limit(30));
    const snap=await getDocs(q);
    reviewableReadings=snap.docs.map(d=>({id:d.id,...d.data()}));
  }catch(e){reviewableReadings=[];}
  return reviewableReadings;
}
function populateReadingSelect(select,readings){
  if(!select)return;
  select.innerHTML='<option value="">후기를 남길 이용 기록을 선택해주세요</option>';
  if(!readings.length){select.innerHTML+='<option value="" disabled>저장된 분석 기록이 없습니다</option>';return;}
  readings.forEach((r,index)=>{
    const service=TYPE_LABELS[r.type]||r.serviceLabel||'정월재 분석';
    const title=(r.title||service).replace(/\s+/g,' ').trim();
    const date=formatDate(r.createdAt);
    const option=document.createElement('option');
    option.value=r.id;option.dataset.service=service;option.textContent=`${service} · ${title}${date?` · ${date}`:''}`;
    select.appendChild(option);
  });
}
onAuthStateChanged(auth,async user=>{currentUser=user;await loadProfile(user);if(user)await loadReviewableReadings(user);});

function subscribeReviews(callback,onError){const q=query(collection(db,'reviews'),orderBy('createdAt','desc'));return onSnapshot(q,snap=>callback(withSamples(snap.docs.map(d=>({id:d.id,...d.data()})))),error=>{callback(SAMPLE_REVIEWS);onError?.(error);});}

function initPage(){
  if(document.body.dataset.reviewsPage!=='true')return;
  const grid=document.querySelector('[data-review-grid]'),countNode=document.querySelector('[data-review-count]'),avgNode=document.querySelector('[data-review-average]'),pageNode=document.querySelector('[data-review-page]'),filter=document.querySelector('[data-review-filter]'),prev=document.querySelector('[data-review-prev]'),next=document.querySelector('[data-review-next]'),open=document.querySelector('[data-review-open]'),closeButtons=document.querySelectorAll('[data-review-close]'),composer=document.querySelector('[data-review-composer]'),form=document.querySelector('[data-review-form]'),status=document.querySelector('[data-review-status]'),readingSelect=document.querySelector('[data-review-reading]');
  let page=1,items=[];
  const redraw=()=>{const category=filter?.value||'all';const filtered=category==='all'?items:items.filter(item=>item.service===category);const totalPages=Math.max(1,Math.ceil(filtered.length/PAGE_SIZE));page=Math.min(page,totalPages);const chunk=filtered.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE);grid.innerHTML=chunk.length?chunk.map(cardMarkup).join(''):'<div class="review-empty">아직 등록된 후기가 없습니다. 첫 후기를 남겨주세요.</div>';countNode.textContent=String(items.length);avgNode.textContent=items.length?average(items).toFixed(1):'—';pageNode.textContent=`${page} / ${totalPages}`;prev.disabled=page<=1;next.disabled=page>=totalPages;};
  subscribeReviews(nextItems=>{items=nextItems;redraw();status.textContent='';},()=>{status.textContent='후기 데이터를 불러오는 중 문제가 있어 기본 후기 목록을 표시하고 있습니다.';});
  ...
  open?.addEventListener('click',async()=>{
    ...
    const readings=await loadReviewableReadings(currentUser);populateReadingSelect(readingSelect,readings);
    composer.hidden=false;
    status.textContent=readings.length?'최근 저장한 분석 중 후기를 남길 기록을 골라주세요.':'저장된 분석 기록이 없습니다. 분석 결과를 정월록에 저장한 뒤 후기를 남길 수 있습니다.';
  ...
  form?.addEventListener('submit',async event=>{
    ...
    if(!currentUser){location.href='./login.html?next=./reviews.html';return;}
    if(!currentProfile)await loadProfile(currentUser);
    const selected=readingSelect?.selectedOptions?.[0];
    const service=selected?.dataset?.service||'';
    const data=new FormData(form),content=String(data.get('content')||'').trim(),rating=Number(data.get('rating')||5);
    if(!selected?.value||!service){status.textContent='후기를 남길 이용 기록을 먼저 선택해주세요.';readingSelect?.focus();return;}
    ...
    try{await addDoc(collection(db,'reviews'),{uid:currentUser.uid,authorLabel:anonymousLabel(currentProfile),service,rating,content,status:'published',createdAt:serverTimestamp()});form.reset();composer.hidden=true;status.textContent='후기가 등록되었습니다. 이름은 공개되지 않고 익명 정보로 표시됩니다.';page=1;}
    catch(error){status.textContent='후기를 등록하지 못했습니다. 잠시 후 다시 시도해주세요.';}
    finally{submit.disabled=false;submit.textContent='후기 등록하기';}
  });
}

function initPreview(){const section=document.querySelector('#reviews');if(!section||document.body.dataset.reviewsPage==='true')return;subscribeReviews(items=>{const preview=items.slice(0,4);section.innerHTML=`<div class="container"><div class="section-head center-head"><p class="section-label">REVIEW</p><h2>정월재를 경험한<br>사람들의 기록</h2><p>서비스를 이용한 뒤 남긴 후기를 익명으로 모아봅니다.</p></div><div class="review-overview"><div><span>누적 후기</span><strong>${items.length}</strong><small>현재 표시 중인 후기</small></div><div><span>평균 평점</span><strong>${items.length?average(items).toFixed(1):'—'}</strong><small>${items.length?stars(Math.round(average(items))):'아직 후기 없음'}</small></div><div><span>후기 등록</span><strong class="review-word">WRITE</strong><small>이용 기록 선택 후 작성</small></div></div><div class="review-grid">${preview.length?preview.map(cardMarkup).join(''):'<div class="review-empty">아직 등록된 후기가 없습니다.</div>'}</div><div class="review-preview-actions"><a class="button button-accent" href="./reviews.html">후기 전체보기 · 등록하기</a></div></div>`;},()=>{});}
