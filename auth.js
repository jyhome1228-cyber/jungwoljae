...
function friendlyAuthError(error,stage='auth'){
  ...
  if(code.includes('api-key-not-valid')||code.includes('invalid-api-key')) return 'Firebase Web API 키가 현재 앱에서 유효하지 않습니다. 최신 Web App 설정을 다시 불러온 뒤 재시도해주세요.';
...
function setupLogin(){
  ...
  const status=form.querySelector('[data-login-status]');if(!configured){setStatus(status,'Firebase 설정값이 아직 연결되지 않았습니다.','error');return;}
  form.addEventListener('submit',async(event)=>{
    ...
    try{await signInWithEmailAndPassword(auth,email,password);const params=new URLSearchParams(location.search);location.href=params.get('next')||'./mypage.html';}
    catch(error){setStatus(status,friendlyAuthError(error),'error',errorCode(error));}
...
function setupSignup(){
  ...
  form.addEventListener('submit',async(event)=>{
    event.preventDefault();['userId','email','password','passwordConfirm','name','birthDate','birthTime','gender','city','privacyConsent'].forEach(k=>error(k));setStatus(status,'','error');let valid=true;
    const userId=field('userId').value.trim();const email=field('email').value.trim();const password=field('password').value;const passwordConfirm=field('passwordConfirm').value;
    ...
    try{
      credential=await createUserWithEmailAndPassword(auth,email,password);authCreated=true;await updateProfile(credential.user,{displayName:field('name').value.trim()});const uid=credential.user.uid;const usernameRef=doc(db,'usernames',userId.toLowerCase());const userRef=doc(db,'users',uid);
      try{await runTransaction(db,async(tx)=>{const usernameSnap=await tx.get(usernameRef);if(usernameSnap.exists())throw new Error('USERNAME_TAKEN');tx.set(usernameRef,{uid,createdAt:serverTimestamp()});tx.set(userRef,{userId,email,name:field('name').value.trim(),birthDate,birthTime,birthTimeUnknown:Boolean(timeUnknown?.checked),calendarType:field('calendarType').value,gender:field('gender').value,city:field('city').value.trim(),privacyConsent:true,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});});}
      catch(fsError){if(fsError?.message==='USERNAME_TAKEN')throw fsError;fsError.stage='firestore';throw fsError;}
      setStatus(status,'회원등록이 완료되었습니다. 마이페이지로 이동합니다.','success');setTimeout(()=>{location.href='./mypage.html';},500);
    }catch(err){if(authCreated&&credential?.user){try{await deleteUser(credential.user);}catch(e){}}if(err?.message==='USERNAME_TAKEN'){error('userId','이미 사용 중인 아이디입니다.');setStatus(status,'이미 사용 중인 아이디입니다.','error','USERNAME_TAKEN');}else setStatus(status,friendlyAuthError(err,err?.stage),'error',errorCode(err));}
    finally{button.disabled=false;button.textContent='회원등록';}
  });
}

async function setupMyPage(user){
  const root=document.querySelector('[data-mypage]');if(!root)return;const status=root.querySelector('[data-mypage-status]');if(!configured){status.textContent='Firebase 설정값을 연결하면 회원 정보가 표시됩니다.';return;}if(!user){location.href='./login.html?next=./mypage.html';return;}
  try{const snap=await getDoc(doc(db,'users',user.uid));const data=snap.exists()?snap.data():{};const values={email:user.email||data.email||'—',userId:data.userId||'—',name:data.name||user.displayName||'—',birthDate:data.birthDate||'—',birthTime:data.birthTimeUnknown?'모름':(data.birthTime||'—'),calendarType:data.calendarType==='lunar'?'음력':'양력',gender:data.gender==='male'?'남성':data.gender==='female'?'여성':'—',city:data.city||'—'};Object.entries(values).forEach(([key,value])=>{const el=root.querySelector(`[data-profile="${key}"]`);if(el)el.textContent=value;});status.textContent='';}
