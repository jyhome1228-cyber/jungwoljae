...
function friendlyAuthError(error,stage='auth'){
  ...
  if(code.includes('api-key-not-valid')||code.includes('invalid-api-key')) return 'Firebase Web API 키가 현재 앱에서 유효하지 않습니다. Firebase 프로젝트 설정의 Web App 구성과 Google Cloud API 키 제한을 확인해주세요.';
...
function populateBirthSelectors(form){
  const year=form.querySelector('#signup-birth-year');
  const month=form.querySelector('#signup-birth-month');
  const day=form.querySelector('#signup-birth-day');
  const meridiem=form.querySelector('#signup-birth-meridiem');
  const hour=form.querySelector('#signup-birth-hour');
  const minute=form.querySelector('#signup-birth-minute');
...
function setupSignup(){
  ...
  const birth=populateBirthSelectors(form);
  ...
  const syncTimeUnknown=()=>{
    ...
    [birth.meridiem,birth.hour,birth.minute].forEach(el=>{if(el){el.disabled=disabled;if(disabled)el.value='';}});
  };
  ...
  form.addEventListener('submit',async(event)=>{
    ...
    const birthDate=(birth.year?.value&&birth.month?.value&&birth.day?.value)?`${birth.year.value}-${birth.month.value}-${birth.day.value}`:'';
    let birthTime='';
    if(!timeUnknown?.checked && birth.meridiem?.value && birth.hour?.value && birth.minute?.value){
      let h=Number(birth.hour.value)%12;
      if(birth.meridiem.value==='pm') h+=12;
      birthTime=`${String(h).padStart(2,'0')}:${birth.minute.value}`;
    }
    ...
    try{
      ...
      try{
        ...
        if(fsError?.message==='USERNAME_TAKEN') throw fsError;
        fsError.stage='firestore';
        throw fsError;
      }
      ...
      if(err?.message==='USERNAME_TAKEN'){
        ...
        setStatus(status,'이미 사용 중인 아이디입니다.','error','USERNAME_TAKEN');
      }else{
